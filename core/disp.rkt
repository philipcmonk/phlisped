...
(define VAR2MAX 255)
(define VAR3MAX 255)
(define (set-VAR1 v) (set! VAR1 v))
(define (set-VAR2 v) (set! VAR2 v))
(define (set-VAR3 v) (set! VAR3 v))
...
(define Trees (list
               (apply whole-tree 
                      (let* ((dummy-n (node '(0 'dummy "dummy" () () () () ()) '() (delay '()) (lambda (_) "t")))
                       ...
                       (list dummy-n (lambda (a) '()) dummy-utterance (set) '() 0 0 0 0 (list-ref v11ns 3) 0 0 1)))
               (apply whole-tree 
...
(define (paint-bar u)
 (with
  ...
  (paint-utterance-data (x y)
   (gl-push-matrix)
   (gl-translate x y 0)
   (when (cartesian-utterance? u) (ftglRenderFont Font (format "x:  ~a" (cartesian-utterance-x u)) 65535))
   (gl-translate 100 0 0)
   (when (cartesian-utterance? u) (ftglRenderFont Font (format "y:  ~a" (cartesian-utterance-y u)) 65535))
   (gl-translate -100 -20 0)
   (when (cartesian-utterance? u) (ftglRenderFont Font (format "w:  ~a" (cartesian-utterance-w u)) 65535))
   (gl-translate 100 0 0)
   (when (cartesian-utterance? u) (ftglRenderFont Font (format "h:  ~a" (cartesian-utterance-h u)) 65535))
   (gl-translate -100 -20 0)
   (when (cartesian-utterance? u) (ftglRenderFont Font (format "text-w:  ~a" (cartesian-utterance-text-w u)) 65535))
   (gl-translate 100 0 0)
   (when (cartesian-utterance? u) (ftglRenderFont Font (format "text-h:  ~a" (cartesian-utterance-text-h u)) 65535))
   (gl-translate -100 -20 0)
 ...
 (let ((tree
  (let* ((n (root->node root childfunc '()))
   ...
   (whole-tree
    ...
    0
    1))))
 (set-whole-tree-utterance-tree! tree ((v11n-node->v11n-utterance (list-ref v11ns 3)) (whole-tree-n-tree tree) tree))
