...
;                                (< (whole-tree-zoom tree) 1) '()
                               (draw-text
                                text
                                (* (whole-tree-zoom tree) (center x w (/ (- text-w PADDING) (whole-tree-zoom tree)) (- (whole-tree-offset-x tree)) (/ (whole-tree-w tree) (whole-tree-zoom tree))))
                                (* (whole-tree-zoom tree) (+ text-h -3 (center y h (/ text-h (whole-tree-zoom tree)) (- (whole-tree-offset-y tree)) (/ (whole-tree-h tree) (whole-tree-zoom tree)))))
                                (car clr)
                                tree)
                               '())
                              (for-each (lambda (arg) (utterance-paint arg tree)) args)))))
                      ...
                      (lambda (n x y w row siblings tree)
                       (let node->utterance ((n n) (x x) (y y) (w (whole-tree-w tree)) (h (whole-tree-h tree)) (dir 'vertical) (row row) (siblings siblings) (tree tree))
                        (let ((children
